...
export function AreaChartD3({ data, height = 300 }: AreaChartD3Props) {
  ...
  useEffect(() => {
    ...
    const area = d3
      ...
      .y0(chartHeight)
      .y1((d) => yScale(Number.parseFloat(d[1])))
    ...
    // Add areas for each series
    data.data.result.forEach((series: any, i: number) => {
      const color = colors[i % colors.length]

      // Add area
      chart.append("path").datum(series.values).attr("fill", color).attr("fill-opacity", 0.2).attr("d", area)
